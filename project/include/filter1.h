...
class Filter{
public:
    virtual Image* apply(Image original[]) = 0;
};
...
public:
    ...
    Image* apply(Image original[]) override {
...
public:
    ...
    Image* apply(Image original[]) override {
        ...
        filtered1 = intensity;
        filtered2 = direction;
        Image filtered[2] = {filtered1, filtered2};
        return *filtered;
...
public:
    ...
    Image* apply(Image original[]) override { // (intensity, direction), (filtered)
        Image filtered = original[0];
        Image intensity = original[0];
        Image direction = original[1];
